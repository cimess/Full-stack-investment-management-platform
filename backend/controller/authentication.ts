...
.json({success:true,message:"user created successfully. Please check your email to verify your account."
})
}catch(err:any){
logger.error(err);
  if (err.code === 'P2002') {
...
return res.status(200).json({success:true,message:"user logged in successfully",data:{id,roles,fullname,email_user,username,manager_id}})
})
  }catch(err:any){
    logger.error(err);
    return next(createError(500,"Internal Server Error"));
...
export const logoutUser=async(req:AuthRequest,res:Response,next:NextFunction)=>{
  ...
  const tokenExists = await prisma.refreshToken.deleteMany({
  where: {
        token,
    user_id:userId }
});
