...
const DetailsModal = ({ item, onClose,targetPath="/signup" }:{item:StockCardProps,onClose:()=>void,targetPath?:string}) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Card - Glass Neomorphic Style */}
      <div className="relative w-full max-w-4xl bg-gradient-to-br from-gray-900/90 to-black/90 backdrop-blur-xl border border-white/10 rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-300 ring-1 ring-white/5">
        ...
        <div className="flex items-center justify-between p-5 sm:p-8 border-b border-white/5 bg-white/5 backdrop-blur-sm">
          ...
          <button
            onClick={onClose}
            className="group p-2 rounded-full bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/20 text-gray-400 hover:text-white transition-all duration-300 shadow-lg hover:shadow-emerald-500/20"
          >
        ...
        <div className="overflow-y-auto p-5 sm:p-8 space-y-6 sm:space-y-8 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={idx} className="group p-4 rounded-2xl bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all duration-300 hover:-translate-y-1 shadow-lg hover:shadow-xl">
                <div className="flex items-center gap-2 mb-3 text-gray-400 text-sm group-hover:text-emerald-400 transition-colors">
                  <stat.icon className="w-4 h-4" />
                  <span>{stat.label}</span>
                </div>
                <p className="font-semibold text-white truncate text-sm md:text-base" title={stat.value}>
                  {stat.value || 'N/A'}
                </p>
              </div>
            ))}
          ...
          <div className="p-4 sm:p-6 rounded-2xl bg-gradient-to-br from-white/5 to-transparent border border-white/5 shadow-inner">
           ...
           {/* Additional Info */}
           {item.stats?.more && (
            <div className="p-5 rounded-2xl bg-blue-500/5 border border-blue-500/10 text-blue-200/80 text-sm backdrop-blur-sm">
              <p>{item.stats.more}</p>
            </div>
          )}
        ...
        {/* Footer Actions */}
        <div className="p-6 border-t border-white/5 bg-black/20 backdrop-blur-md flex justify-end gap-4">
          <button
            onClick={onClose}
            className="px-6 py-3 rounded-xl font-semibold text-gray-300 hover:text-white hover:bg-white/5 transition-all duration-300 border border-transparent hover:border-white/10"
          >
            Close
          </button>
          <button
